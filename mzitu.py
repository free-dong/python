...
class mzitu():
    ...
    def all_url(self, url):
        ...
        all_a = BeautifulSoup(html.text, 'html.parser').find('ul', class_='archives').find_all('a')
        for a in all_a:
            title = a.get_text()
            old = '早期图片'
            if title ==old:
            ...
            path_ = path.strip() # 去除首尾空格
            isExists = os.path.exists(os.path.join("E:\codes\python\pachong\mzitu", path_))
    ...
    def mkdir(self, path): ##这个函数创建文件夹
        path = path.strip()
        isExists = os.path.exists(os.path.join("E:\codes\python\pachong\mzitu", path))
        if not isExists:
            print(u'新建文件：', path)
            os.makedirs(os.path.join("E:\codes\python\pachong\mzitu", path))
            os.chdir(os.path.join("E:\codes\python\pachong\mzitu", path)) ##切换到目录
            return True
        else:
            print(u'文件：', path, u'已经存在')
            return False
